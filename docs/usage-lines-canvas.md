...
```javascript
import { LinesCanvas } from './src/lib/lines-canvas.ts'
...
|----------------------|--------|-------------|------------------------------------------------|
| `render-width`       | int    | 1024        | Canvas render width in pixels                  |
| `render-height`      | int    | 1024        | Canvas render height in pixels                 |
...
| `easing`             | string | Cubic.InOut | Easing function (e.g., `Cubic.InOut`)          |
| `background`         | string | transparent | Background color (hex, named, etc.)            |
| `animation-duration` | int    | 0           | Animation duration in ms                       |
| `animation-easing`   | string | Cubic.InOut | Animation easing function                      |
| `line-cap`           | string | round       | Line end cap style (`round`, `square`, `butt`) |
| `line-join`          | string | round       | Line join style (`round`, `bevel`, `miter`)    |
| `autoplay`           | string | false       | Animation mode (`true`, `loop`, or `false`)    |
...
| `renderer`  | `Renderer`                 | The renderer controller                 |
...
The canvas automatically fills its container:
...
<script>
  document.getElementById('example').addEventListener('animated-in', () => {
    // Trigger next action
    document.getElementById('example').renderer.animateWipeOut()
  })
</script>